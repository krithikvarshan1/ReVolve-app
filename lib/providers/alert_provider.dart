...
class AlertProvider with ChangeNotifier {
  ...
  Future<void> createAlertFromSensorData(SensorData data) async {
    ...
    if (data.dust > 250.0) {
      alerts.add(Alert(
        id: 'dust-${DateTime.now().millisecondsSinceEpoch}',
        title: 'High Dust Alert',
        message: 'Dust concentration exceeded safe threshold: ${data.dust.toStringAsFixed(0)} ug/m3',
        severity: AlertSeverity.medium,
        timestamp: DateTime.now(),
        deviceId: data.deviceId,
        metadata: {'sensor': 'dust', 'value': data.dust},
      ));
    }
